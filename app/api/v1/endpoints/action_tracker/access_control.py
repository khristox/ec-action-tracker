"""
Meeting Management API - RBAC Permission-Based Version

Features:
- Uses actual RBAC permissions from roles (meetings.edit, meetings.delete, etc.)
- Email-based participant identification (for access control)
- Granular stats (created vs participated)
- Permissions control actions, not just roles
...
class MeetingPermissions(str, Enum):
    """Standard meeting permissions used in your RBAC system"""
    VIEW_MEETING = "meetings.view"
    CREATE_MEETING = "meetings.create"
    EDIT_MEETING = "meetings.edit"
    DELETE_MEETING = "meetings.delete"
    MANAGE_PARTICIPANTS = "meetings.manage_participants"
    VIEW_STATS = "meetings.view_stats"
...
class PermissionChecker:
    """
    Check user permissions from RBAC system.
    Uses actual permissions assigned to user's roles.
    """
    ...
    async def get_user_permissions(
        ...
        """
        Get all permissions for current user from their roles.
        
        Assumes current_user object has:
        - roles: List of role objects
        - Each role has: permissions: List of permission strings
        
        Returns set of permission strings like:
        ...
        try:
            ...
            for role in current_user.roles:
                if hasattr(role, 'permissions') and role.permissions:
                    # If permissions is a list of strings
                    if isinstance(role.permissions, list):
                        for perm in role.permissions:
                            if isinstance(perm, str):
                                permissions.add(perm)
                            elif hasattr(perm, 'name'):
                                # If it's permission objects with .name attribute
                                permissions.add(perm.name)
                    # If permissions is a relationship/query
                    elif hasattr(role.permissions, '__iter__'):
                        for perm in role.permissions:
                            if isinstance(perm, str):
                                permissions.add(perm)
                            elif hasattr(perm, 'name'):
            ...
            logger.debug(f"User {current_user.id} has permissions: {permissions}")
            return permissions
    ...
    async def has_permission(
        ...
        permissions = await PermissionChecker.get_user_permissions(current_user)
        has_perm = required_permission in permissions
        
        if not has_perm:
            logger.warning(
                f"User {current_user.id} missing permission '{required_permission}'. "
                f"Has: {permissions}"
            )
    ...
    async def has_any_permission(
        ...
        user_perms = await PermissionChecker.get_user_permissions(current_user)
    ...
    def require_permission(permission: str):
        """Dependency to enforce permission in routes"""
        async def check_permission(current_user: User = Depends(deps.get_current_user)):
            has_perm = await PermissionChecker.has_permission(current_user, permission)
            if not has_perm:
                raise HTTPException(
...
class AccessControl:
    ...
    Permissions control ACTIONS separately.
    """
...
class StatsComputer:
    """Compute meeting statistics with RBAC awareness"""
    ...
    async def compute_user_stats(
        ...
        Only includes meetings user has access to.
        
        Returns:
        {
            "created": { "total": 5, "upcoming": 2, ... },
            "participating": { "total": 10, "upcoming": 5, ... },
            "total": { "total": 15, "upcoming": 7, ... },
            "user_id": "...",
            "cached_at": "..."
        }
        """
...
async def get_meeting_stats(
    ...
    - Uses email-based participant matching
...
async def list_meetings(
    ...
    """
    List meetings accessible to user (permission-based filtering).
    
    ✅ Improvements:
    - Shows only meetings user created or participates in
    - Email-based participant matching
    - Respects meeting visibility
    """
    ...
    try:
        async def fetch_meetings():
            ...
            # Only show meetings user created or participates in
...
async def create_meeting(
    ...
    db: AsyncSession = Depends(deps.get_db),
    current_user: User = Depends(PermissionChecker.require_permission(MeetingPermissions.CREATE_MEETING.value)),
):
    """
    Create meeting.
    
    ✅ Requires permission: 'meetings.create'
    """
...
async def get_meeting(
    ...
    - Shows what actions user can perform (based on permissions)
    """
    
    try:
        # Check access (can user view this meeting?)
        ...
        # Get user's permissions for actions on this meeting
        user_permissions = await PermissionChecker.get_user_permissions(current_user)
        
        response = build_meeting_response(meeting)
        response["user_actions"] = {
            "can_edit": MeetingPermissions.EDIT_MEETING.value in user_permissions,
            "can_delete": MeetingPermissions.DELETE_MEETING.value in user_permissions,
            "can_manage_participants": MeetingPermissions.MANAGE_PARTICIPANTS.value in user_permissions,
        }
...
async def update_meeting(
    ...
    meeting_in: MeetingCreate,
    db: AsyncSession = Depends(deps.get_db),
    current_user: User = Depends(PermissionChecker.require_permission(MeetingPermissions.EDIT_MEETING.value)),
):
    """
    Update meeting.
    
    ✅ Requires permission: 'meetings.edit'
    """
...
async def delete_meeting(
    meeting_id: UUID,
    db: AsyncSession = Depends(deps.get_db),
    current_user: User = Depends(PermissionChecker.require_permission(MeetingPermissions.DELETE_MEETING.value)),
):
    """
    Delete meeting (soft delete).
    
    ✅ Requires permission: 'meetings.delete'
    """
...
async def list_participants(
    ...
    ✅ Access control: User must be able to access meeting
    """
...
async def add_participant(
    ...
    db: AsyncSession = Depends(deps.get_db),
    current_user: User = Depends(PermissionChecker.require_permission(MeetingPermissions.MANAGE_PARTICIPANTS.value)),
):
    """
    Add participant to meeting.
    
    ✅ Requires permission: 'meetings.manage_participants'
    """
