...
class CRUDMeeting(CRUDBase[Meeting, MeetingCreate, MeetingUpdate]):
    ...
    async def create_with_participants(
        ...
        if obj_in.participant_list_id:
            ...
            participant_list_obj = result.scalar_one_or_none()
            
            if participant_list_obj:
                for template_participant in participant_list_obj.participants:
...
class CRUDMeetingAction(CRUDBase[MeetingAction, MeetingActionCreate, MeetingActionUpdate]):
    ...
    async def get_status_history(
        self, db: AsyncSession, action_id: UUID
    ) -> List[ActionStatusHistory]:
        result = await db.execute(
            select(ActionStatusHistory)
            .where(ActionStatusHistory.action_id == action_id)
...
class CRUDDashboard:
    ...
    async def get_actions_summary(self, db: AsyncSession) -> Dict[str, Any]:
        """Get actions summary for dashboard"""
        total_result = await db.execute(select(func.count()).select_from(MeetingAction))
        ...
        in_progress_result = await db.execute(
            ...
            .where(
                MeetingAction.completed_at.is_(None),
                MeetingAction.start_date.isnot(None)
            )
        )
        ...
        pending_result = await db.execute(
            ...
            .where(
                MeetingAction.completed_at.is_(None),
                MeetingAction.start_date.is_(None)
            )
        )
        ...
        blocked_result = await db.execute(
            select(func.count()).select_from(MeetingAction)
            .where(MeetingAction.status == "blocked")
        )
    ...
    async def get_meetings_by_month(self, db: AsyncSession, year: int) -> Dict[str, Any]:
        ...
        result = await db.execute(
            ...
            .where(extract('year', Meeting.meeting_date) == year)
            .group_by('month')
            .order_by('month')
        )
        
        meetings_by_month = {int(row[0]): row[1] for row in result.all()}
        return {"year": year, "meetings_by_month": meetings_by_month}
    
    async def get_actions_by_status(self, db: AsyncSession) -> Dict[str, Any]:
        ...
        result = await db.execute(
            ...
            .group_by(MeetingAction.status)
        )
        
        actions_by_status = {row[0]: row[1] for row in result.all()}
        return {"actions_by_status": actions_by_status}
