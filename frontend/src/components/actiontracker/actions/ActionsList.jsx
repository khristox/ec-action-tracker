...
const ActionsList = () => {
  ...
  const { myTasks, filters, loading } = useSelector((state) => ({
    myTasks: state.actions.myTasks,
    filters: state.actions.filters,
    loading: state.actions.loading || false
  }));
  ...
  // Mount-time fetch with abort
  useEffect(() => {
    const promise = dispatch(fetchMyTasks({ ...filters, page: myTasks.page }));
    return () => {
          ...
          <CardContent sx={{ p: 3 }}>
            ...
            >
              <Stack direction="row" spacing={0.5} alignItems="center">
                <ScheduleIcon sx={{ 
                  fontSize: 14, 
                  color: action.is_overdue ? 'error.main' : 'text.secondary' 
                }} />
                <Typography 
                  variant="caption" 
                  fontWeight={600} 
            ...
            >
              <Box sx={{ flex: { xs: '1 1 100%', sm: '1 1 70%' } }}>
                <Stack direction="row" justifyContent="space-between" mb={1}>
                  <Typography variant="caption" fontWeight={800} sx={{ color: 'text.secondary' }}>
                    Progress
                  </Typography>
                  <Typography variant="caption" fontWeight={800} sx={{ color: style.color }}>
                    {action.overall_progress_percentage}%
                  </Typography>
                </Stack>
                <LinearProgress 
                  variant="determinate" 
    ...
    <Box sx={{ bgcolor: 'background.default', minHeight: '100vh', py: 4 }}>
      <Container maxWidth="lg">
        ...
        {loading && !myTasks.items.length ? (
          ...
          <Box>
            {myTasks.items.map((action, index) => (
              <ActionCard key={action.id} action={action} index={index} />
            ))}
            
            {myTasks.items.length === 0 && !loading && (
              <Paper sx={{ p: 4, textAlign: 'center', borderRadius: 3 }}>
                <Typography variant="h6" color="text.secondary">
                  No tasks assigned to you
                </Typography>
                <Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>
                  When actions are assigned to you, they'll appear here
                </Typography>
              </Paper>
            )}
            ...
            {myTasks.totalPages > 1 && (
              <Box sx={{ display: 'flex', justifyContent: 'center', mt: 4 }}>
                <Pagination 
                  ...
                  page={myTasks.page} 
                  onChange={(e, v) => {
                    dispatch(setFilters({ page: v }));
                    dispatch(fetchMyTasks({ ...filters, page: v }));
                  }} 
                  color="primary"
                  shape="rounded"
                />
