...
const MeetingInfoCard = memo(({ meeting, isMobile, onUpdateLink, onJoinMeeting, viewMode }) => {
  ...
  const hasKeyPersons = !isSimple && (meeting?.chairperson_name || meeting?.facilitator);
  ...
  return (
    ...
    >
      <CardContent sx={{ p: { xs: 2, sm: 2.5, md: 2.5 } }}>
        <Stack
        ...
        >
          <Typography variant={isMobile ? 'h6' : 'h5'} fontWeight={800} sx={{ fontSize: { md: '1.3rem' } }}>
            {meeting?.title}
          </Typography>
        ...
        {/* Primary row: two wider columns rather than four narrow ones —
            reads better on small-desktop widths (~1000-1280px) where four
            equal columns leave everything cramped. */}
        <Grid container spacing={{ xs: 2, sm: 2.5 }}>
          ...
          {/* Location/Platform — the Agenda now lives inside this box instead
              of as its own large section, so the two related pieces of
              "where and what" sit together and collapse/expand as one unit. */}
        ...
        {isSimple && (meeting?.chairperson_name || meeting?.facilitator) && (
          <>
            <Divider sx={{ mt: 2, mb: 1.5 }} />
            <Stack direction="row" spacing={3} sx={{ flexWrap: 'wrap', rowGap: 1 }}>
              {meeting?.chairperson_name && (
                <Typography variant="body2"><Box component="span" color="text.secondary">Chair:</Box> <strong>{meeting.chairperson_name}</strong></Typography>
              )}
              {meeting?.facilitator && (
                <Typography variant="body2"><Box component="span" color="text.secondary">Secretary:</Box> <strong>{meeting.facilitator}</strong></Typography>
              )}
            </Stack>
          </>
        )}
        ...
        {isSimple && (
          <Box sx={{ mt: 1.5, pt: 1.5, borderTop: `1px dashed ${isDarkMode ? DARK.border : '#E5E7EB'}` }}>
            <Typography variant="caption" color="text.secondary" sx={{ fontStyle: 'italic' }}>
              Switch to <strong>Detailed</strong> view to see the agenda and description.
            </Typography>
          </Box>
        )}
      </CardContent>
