...
// Optimized direct-path icon imports to enhance tree-shaking performance
...
// Code-splitting with Lazy Loading: Sub-tabs are compiled independently and loaded on-demand
...
// Minimal critical localized configurations
...
const MeetingDetail = () => {
  ...
  const canRecord = useMemo(() => ['started', 'ongoing', 'in_progress'].includes(normalizedMeeting?.status?.short_name), [normalizedMeeting]);
  const emailSentCount = useMemo(() => emailNotifications.filter((n) => n.status === 'successful').length, [emailNotifications]);
  const emailFailedCount = useMemo(() => emailNotifications.filter((n) => n.status === 'failed').length, [emailNotifications]);
  ...
  const fetchMeetingData = useCallback(async () => {
    ...
    try {
      ...
      const res = await api.get('/notifications', { params: { meeting_id: id, channel: 'email', limit: 50 }, signal: abortControllerRef.current.signal });
      if (isMountedRef.current) setEmailNotifications(res.data?.items || []);
      await dispatch(fetchActionTrackerAttributes());
      if (isMountedRef.current) { setInitialLoadComplete(true); setLoadingTimeout(false); }
    } catch (err) {
      if (err.name !== 'AbortError' && isMountedRef.current) setInitialLoadComplete(true);
    }
  }, [id, dispatch]);

  const handleRefresh = useCallback(() => {
    setShowNotFound(false); setLoadingTimeout(false); setInitialLoadComplete(false);
    fetchAttemptedRef.current = false; fetchMeetingData();
  }, [fetchMeetingData]);

  useEffect(() => {
    isMountedRef.current = true;
    if (id && !fetchAttemptedRef.current) { fetchAttemptedRef.current = true; fetchMeetingData(); }
    return () => {
      isMountedRef.current = false;
      if (abortControllerRef.current) abortControllerRef.current.abort();
      dispatch(clearMeetingState()); dispatch(clearNotificationError()); dispatch(clearLastNotificationResult());
    };
  }, [id, dispatch, fetchMeetingData]);

  useEffect(() => {
    if (!loading || initialLoadComplete) { setLoadingTimeout(false); return; }
    const timer = setTimeout(() => { if (!initialLoadComplete && isMountedRef.current) setLoadingTimeout(true); }, NOT_FOUND_DELAY_MS);
  ...
  useEffect(() => {
    if (lastNotificationResult && isMountedRef.current) {
      ...
      setNotificationDialogOpen(false); handleRefresh(); dispatch(clearLastNotificationResult());
    }
  }, [lastNotificationResult, dispatch, handleRefresh]);

  useEffect(() => {
    if (notificationError && isMountedRef.current) { setSnackbar({ open: true, message: notificationError, severity: 'error' }); dispatch(clearNotificationError()); }
  }, [notificationError, dispatch]);

  useEffect(() => { if (id) localStorage.setItem(`${TAB_STORAGE_KEY_PREFIX}${id}`, String(tabValue)); }, [tabValue, id]);
  useEffect(() => { if (id) localStorage.setItem(`${TAB_ORDER_STORAGE_KEY_PREFIX}${id}`, JSON.stringify(tabOrder)); }, [tabOrder, id]);

  const getStatusValue = useCallback(() => {
    const s = normalizedMeeting?.status;
    if (!s) return '';
    return s.short_name ? s.short_name.toLowerCase() : (typeof s === 'string' && s.includes('_') ? s.split('_').pop().toLowerCase() : String(s).toLowerCase());
  }, [normalizedMeeting]);
  ...
  const getStatusDisplay = useCallback(() => {
    const s = normalizedMeeting?.status;
    if (!s) return 'Unknown';
    return s.short_name ? s.short_name.charAt(0).toUpperCase() + s.short_name.slice(1) : s.name || 'Unknown';
  }, [normalizedMeeting]);

  const handleBack = useCallback(() => navigate('/meetings'), [navigate]);
  const handleEdit = useCallback(() => canUpdateMeeting ? navigate(`/meetings/${id}/edit`) : setSnackbar({ open: true, message: 'Missing permissions to edit', severity: 'error' }), [navigate, id, canUpdateMeeting]);
  const handleRecord = useCallback(() => hasPermission(userPermissions, 'meeting:record') ? navigate(`/meetings/${id}/record`) : setSnackbar({ open: true, message: 'Missing permissions to record', severity: 'error' }), [navigate, id, userPermissions]);
  ...
  const handleJoinMeeting = useCallback(() => {
    if (isOnlineMeeting && hasMeetingLink) {
      ...
      setSnackbar({ open: true, message: normalizedMeeting?.location_text ? `📍 Physical: ${normalizedMeeting.location_text}` : 'No location specified', severity: 'info' });
    }
  ...
  const handleNotifyClick = useCallback(() => canSendNotifications ? setNotificationDialogOpen(true) : setSnackbar({ open: true, message: 'Notification permission error', severity: 'error' }), [canSendNotifications]);
  const handleSendNotifications = useCallback((data) => dispatch(sendMeetingNotifications({ meetingId: id, notificationData: data })), [id, dispatch]);
  const handleEmailHistoryOpen = useCallback(() => navigate(`/meetings/${id}/notifications`), [navigate, id]);
  ...
  const handleStatusSelect = (val) => { setSelectedStatus(val); setStatusDialogOpen(true); setStatusMenuAnchor(null); };
  ...
  const handleStatusUpdate = async () => {
    ...
    try {
      ...
      setStatusDialogOpen(false); setSelectedStatus(''); setStatusComment(''); handleRefresh();
      setSnackbar({ open: true, message: '✅ Meeting status updated!', severity: 'success' });
    } catch (err) {
      setSnackbar({ open: true, message: err.message || 'Status failure', severity: 'error' });
    } finally { setStatusUpdating(false); }
  };

  const handleDeleteClick = () => canDeleteMeeting ? setDeleteDialogOpen(true) : setSnackbar({ open: true, message: 'Missing delete rights', severity: 'error' });

  const handleDelete = async () => {
    setDeleting(true);
    try {
      await dispatch(deleteMeeting(id)).unwrap(); setDeleteDialogOpen(false); navigate('/meetings');
    } catch (err) {
      setLocalError(err.message || 'Deletion failed'); setDeleteDialogOpen(false);
  ...
  const handlePrintPDF = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/v1/meetings/${id}/report/pdf`, { headers: { Authorization: `Bearer ${localStorage.getItem('access_token')}` } });
      const blob = await res.blob();
      const link = document.createElement('a'); link.href = window.URL.createObjectURL(blob);
      link.download = `meeting_report_${id}.pdf`; link.click();
    } catch { setSnackbar({ open: true, message: 'PDF export failure', severity: 'error' }); }
  }, [id, canExportReports]);

  const handleExportJSON = useCallback(async () => {
    if (!canExportReports) return;
    try {
      const res = await fetch(`/api/v1/meetings/${id}/report`, { headers: { Authorization: `Bearer ${localStorage.getItem('access_token')}`, Accept: 'application/json' } });
      const data = await res.json();
      const link = document.createElement('a'); link.href = window.URL.createObjectURL(new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' }));
      link.download = `meeting_${id}.json`; link.click();
    } catch { setSnackbar({ open: true, message: 'JSON export failure', severity: 'error' }); }
  }, [id, canExportReports]);
  ...
  const handleTabDragStart = useCallback((e, val) => {
    ...
    setDraggedTabValue(val); setIsDraggingTab(true);
  }, []);

  const handleTabDragOver = useCallback((e, val) => {
    e.preventDefault(); if (val === PINNED_TAB_VALUE || draggedTabValue === null) return;
    if (dragOverTabValue !== val) setDragOverTabValue(val);
  }, [draggedTabValue, dragOverTabValue]);

  const handleTabDrop = useCallback((e, val) => {
    e.preventDefault(); if (val === PINNED_TAB_VALUE || draggedTabValue === null || draggedTabValue === val) { handleTabDragEnd(); return; }
    setTabOrder((prev) => {
      const order = [...prev]; const from = order.indexOf(draggedTabValue); const to = order.indexOf(val);
      order.splice(from, 1); order.splice(to, 0, draggedTabValue); return order;
    });
    handleTabDragEnd();
  }, [draggedTabValue]);

  const handleTabDragEnd = useCallback(() => { setDraggedTabValue(null); setDragOverTabValue(null); setIsDraggingTab(false); }, []);
  ...
  const visibleTabs = useMemo(() => TABS.filter(t => isAdmin || !t.requiresPermission || hasPermission(userPermissions, t.requiresPermission)), [isAdmin, userPermissions]);
  ...
  const orderedVisibleTabsForMode = useMemo(() => {
    ...
    const ordered = [];
    tabOrder.forEach(v => { if (map.has(v)) { ordered.push(map.get(v)); map.delete(v); } });
    map.forEach(t => ordered.push(t));
    return ordered;
  }, [visibleTabsForMode, tabOrder]);

  const effectiveTabValue = useMemo(() => visibleTabsForMode.some(t => t.value === tabValue) ? tabValue : (visibleTabsForMode[0]?.value ?? tabValue), [visibleTabsForMode, tabValue]);
  ...
  return (
    <Box sx={{ minHeight: '100vh', bgcolor: isDarkMode ? DARK.bg : '#F3F4F6' }}>
      <AppBar position="sticky" elevation={0} sx={{ bgcolor: isDarkMode ? DARK.surfaceAlt : '#FFFFFF', borderBottom: `1px solid ${isDarkMode ? DARK.border : '#E5E7EB'}`, color: 'text.primary' }}>
        <Toolbar sx={{ px: 2, minHeight: 56 }}>
          ...
          <Stack direction="row" spacing={0.5}>
            {!isMobile && (
              <>
                <Tooltip title="PDF Report"><IconButton onClick={handlePrintPDF} size="small"><PictureAsPdfIcon /></IconButton></Tooltip>
                <Tooltip title="Sync Settings"><IconButton onClick={() => setUpdateLinkDialogOpen(true)} size="small"><UpdateIcon /></IconButton></Tooltip>
              </>
            )}
            ...
            <IconButton onClick={handleEmailHistoryOpen} size="small">
              <Badge badgeContent={emailSentCount} color="success"><MarkEmailReadIcon /></Badge>
            </IconButton>
            <IconButton onClick={handleRefresh} size="small"><RefreshIcon /></IconButton>
            <IconButton onClick={handleMoreMenuOpen} size="small"><MoreVertIcon /></IconButton>
          </Stack>
        </Toolbar>
      </AppBar>

      <Container maxWidth="xl" sx={{ py: 3 }}>
        {(error || localError) && <Alert severity="error" sx={{ mb: 3 }}>{localError || 'Network Sync Error'}</Alert>}
        ...
        <Box sx={{ mb: 2, p: 2, borderRadius: 3, border: `1px solid ${isDarkMode ? DARK.border : '#E5E7EB'}`, bgcolor: isDarkMode ? DARK.surface : '#FFFFFF', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <Chip label={getStatusDisplay()} color={getStatusColor()} icon={getStatusIcon()} size="small" sx={{ fontWeight: 600 }} />
        </Box>
        ...
        {visibleTabsForMode.length > 0 && (
          <Paper sx={{ borderRadius: 3, overflow: 'hidden', border: `1px solid ${isDarkMode ? DARK.border : '#E5E7EB'}` }}>
            ...
            <Box sx={{ p: 2.5 }}>
              <TabPanel value={effectiveTabValue} index={0}><MeetingOverviewTab meeting={normalizedMeeting} onUpdateLink={() => setUpdateLinkDialogOpen(true)} onJoinMeeting={handleJoinMeeting} /></TabPanel>
              <TabPanel value={effectiveTabValue} index={1}><MeetingMinutes meetingId={id} meetingStatus={getStatusValue()} onRefresh={handleRefresh} /></TabPanel>
              <TabPanel value={effectiveTabValue} index={2}><MeetingActionsList meetingId={id} meetingStatus={getStatusValue()} onRefresh={handleRefresh} /></TabPanel>
              <TabPanel value={effectiveTabValue} index={3}><ParticipantsTab meetingId={id} participants={participants} onRefresh={handleRefresh} /></TabPanel>
              <TabPanel value={effectiveTabValue} index={4}><MeetingDocuments meetingId={id} /></TabPanel>
              <TabPanel value={effectiveTabValue} index={5}><MeetingHistory meetingId={id} /></TabPanel>
              <TabPanel value={effectiveTabValue} index={6}><MeetingAudit meetingId={id} /></TabPanel>
              <TabPanel value={effectiveTabValue} index={7}><MeetingRecorder meetingId={id} /></TabPanel>
            </Box>
          </Paper>
        )}
      </Container>
      ...
      {isMobile && speedDialActions.length > 0 && (
        <Zoom in={true}><SpeedDial ariaLabel="Actions" sx={{ position: 'fixed', bottom: 16, right: 16 }} icon={<SpeedDialIcon />} onClose={() => setSpeedDialOpen(false)} onOpen={() => setSpeedDialOpen(true)} open={speedDialOpen}>
          {speedDialActions.map((a) => <SpeedDialAction key={a.name} icon={a.icon} tooltipTitle={a.name} onClick={() => handleSpeedDialAction(a.action)} />)}
        </SpeedDial></Zoom>
      )}

      <Menu anchorEl={moreMenuAnchor} open={Boolean(moreMenuAnchor)} onClose={handleMoreMenuClose}>
        <MenuItem onClick={handleShare}><ListItemIcon><ShareIcon fontSize="small" /></ListItemIcon><ListItemText>Share Link</ListItemText></MenuItem>
        <MenuItem onClick={handleStatusMenuOpen}><ListItemIcon>{getStatusIcon()}</ListItemIcon><ListItemText>Change Status</ListItemText></MenuItem>
        {canDeleteMeeting && <MenuItem onClick={handleDeleteClick} sx={{ color: 'error.main' }}><ListItemIcon><DeleteIcon fontSize="small" color="error" /></ListItemIcon><ListItemText>Delete Workspace</ListItemText></MenuItem>}
      </Menu>
      ...
      <Dialog open={statusDialogOpen} onClose={() => setStatusDialogOpen(false)} maxWidth="xs" fullWidth>
        <DialogTitle><Typography variant="subtitle1" fontWeight={700}>Update Status</Typography></DialogTitle>
        <DialogContent><TextField fullWidth label="Reason / Comment" multiline rows={2} value={statusComment} onChange={(e) => setStatusComment(e.target.value)} sx={{ mt: 1 }} /></DialogContent>
        <DialogActions>
          <Button onClick={() => setStatusDialogOpen(false)}>Cancel</Button>
          <Button variant="contained" onClick={handleStatusUpdate} disabled={statusUpdating} sx={{ bgcolor: '#7C3AED' }}>Confirm</Button>
        </DialogActions>
      </Dialog>

      <Dialog open={deleteDialogOpen} onClose={() => setDeleteDialogOpen(false)}>
        <DialogTitle>Confirm Deletion</DialogTitle>
        <DialogContent><Typography variant="body2" color="text.secondary">Permanently delete this workspace tracking file? This metadata action cannot be undone.</Typography></DialogContent>
        <DialogActions>
      ...
      <Snackbar open={snackbar.open} autoHideDuration={SNACKBAR_AUTO_HIDE_MS} onClose={() => setSnackbar(s => ({ ...s, open: false }))}><Alert severity={snackbar.severity} variant="filled">{snackbar.message}</Alert></Snackbar>
    </Box>
  );
};
